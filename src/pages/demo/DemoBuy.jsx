...
function DemoBuy() {
  ...
  const fetchOrders = () => {
    const data = demoStorage.getItem('buy') || [];
    setOrders(data);
  };
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    const newOrder = {
      _id: 'demo-buy-' + Date.now(),
      ...formData,
      createdAt: new Date().toISOString(),
    };
    const currentOrders = demoStorage.getItem('buy') || [];
    demoStorage.setItem('buy', [...currentOrders, newOrder]);
  ...
  const handleDelete = (id) => {
    if (window.confirm('Delete this order?')) {
      const currentOrders = demoStorage.getItem('buy') || [];
      demoStorage.setItem('buy', currentOrders.filter(o => o._id !== id));
      fetchOrders();
    }
  };
        ...
        <div className="bg-gradient-to-r from-amber-900 via-amber-800 to-amber-900 rounded-2xl shadow-2xl p-8 mb-8 border-4 border-amber-700">
          <div className="flex justify-between items-center">
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h1 className="text-4xl font-bold text-amber-50 tracking-wide drop-shadow-lg">Buy Dashboard (Demo)</h1>
                  ...
                  <tr>
                    <td colSpan="6" className="px-6 py-12 text-center text-gray-500">
                      <div className="flex flex-col items-center">
                        ...
                        <p className="text-lg font-semibold">No buy orders found</p>
                        <p className="text-sm mt-1">Click "Add Buy Order" to create your first order</p>
                      </div>
                    </td>
                  </tr>
                ) : (
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-2xl p-8 max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl border-4 border-amber-200">
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-3xl font-bold text-amber-900">Add Buy Order</h2>
                <button
